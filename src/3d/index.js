const getData = (flag) => {
	...
	const render = (data, tag) => {
		...
		!(function (e) {
			function t(t) {
				for (
					var a, u, s = t[0], i = t[1], c = t[2], d = 0, p = [];
					d < s.length;
					d++
				)
					(u = s[d]), r[u] && p.push(r[u][0]), (r[u] = 0);
				for (a in i)
			...
			function (e, t, o) {
				...
				let s = o(6);
				const i = o(7);
				(window.top.forceBallBackground = 16777215),
					(async () => {
						...
						(() => {
							...
							s.nodes.forEach((o) => {
								...
								(s.nodes[0].level = 0),
								(s.edges = s.links.map((o) => {
									let a = e.get(o.source);
									return (
										void 0 === a &&
											(s.nodes.push({ id: o.source }),
											e.set(o.source, t++),
											console.log(a)),
										void 0 === e.get(o.target) &&
											(s.nodes.push({ id: o.target }), e.set(o.target, t++)),
										(s.nodes[e.get(o.target)].level =
											s.nodes[e.get(o.source)].level + 1),
										{ source: e.get(o.source), target: e.get(o.target) }
									);
								})),
								s.nodes.forEach((e) => {
									e.color = ((e) => a.a(o(e)).brighter(1.7).toString())(
										e.level
									);
								});
						})();
						...
						let p = new r.Scene(),
							g = new r.WebGLRenderer({ antialias: !0 });
						g.setSize(e, t),
							(g.shadowMap.enabled = !0),
							g.setClearColor(window.top.forceBallBackground, 1),
							document.body.appendChild(g.domElement);
						let v = new r.PerspectiveCamera(45, e / t, 1, 1e4);
						v.position.set(0, -300, 300),
							v.lookAt(0, 0, 0),
							(window.onresize = () => {
								(v.aspect = window.innerWidth / window.innerHeight),
						...
						c.forEach((e, t) => {
							!(function () {
								...
								(o.fillStyle = '#000000'),
									...
									o.fillText(e.id, 256, 256, 512);
								let a = new r.CanvasTexture(t);
								a.needsUpdate = !0;
								let n = new r.SpriteMaterial({ map: a }),
									u = new r.Sprite(n);
								u.scale.set(30, 30, 1),
									(u.castShadow = !0),
									(u.receiveShadow = !0),
							...
							let o = (function () {
									let o = new r.SphereGeometry(10, 32, 32),
										a = new r.MeshPhongMaterial({ color: e.color }),
										n = new r.Mesh(o, a);
									(n.castShadow = !0),
										(n.receiveShadow = !0),
										(n.frustumCulled = !1);
									{
										let e = c.length,
											o = (50 * e) / 2 / Math.PI;
										n.position.set(
											Math.cos(((2 * Math.PI) / e) * t) * o,
											Math.sin(((2 * Math.PI) / e) * t) * o,
											0
										);
									}
							...
							l.forEach((e) => {
								let t = new r.LineBasicMaterial({
									...
									o = new r.Geometry();
								o.vertices.push(c[e.source].body.position),
									o.vertices.push(c[e.target].body.position),
									o.colors.push(
										c[e.source].body.material.color,
										c[e.target].body.material.color
									);
							...
							(function () {
								let e = new r.AmbientLight(16777215, 0.5);
								e.position.set(0, 10, 0),
									p.add(e),
									(e = new r.SpotLight(16777215, 0.4)).position.set(0, 0, 300),
									(e.castShadow = !0),
						...
						function b() {
							...
							c.forEach((e) => {
								e.body.position.copy(e.pbody.position),
									e.body.quaternion.copy(e.pbody.quaternion);
								let t = v.position
									.clone()
									.sub(e.pbody.position)
									.clampLength(11, 12)
									.add(e.pbody.position);
								e.text.position.copy(t),
									c.forEach((t) => {
										if (t !== e && t.pbody.type === n.Body.DYNAMIC) {
											let o = new r.Vector3()
													.copy(t.pbody.position)
													.distanceTo(e.pbody.position),
												a = 1e5,
												n = new r.Vector3()
													.copy(t.pbody.position)
													.sub(e.pbody.position)
													.clampLength(a / o ** 2, Math.ceil(a / o ** 2));
						...
						y.addEventListener('hoveron', function (e) {
							...
							y.addEventListener('dragstart', function (e) {
								(e.object.data.pbody.type = n.Body.KINEMATIC),
									e.object.data.pbody.updateMassProperties(),
									(f = !0);
							}),
							...
							y.addEventListener('dragend', function (e) {
								(e.object.data.pbody.type = n.Body.DYNAMIC),
									e.object.data.pbody.updateMassProperties(),
									(f = !1),
									!w &&
										e.object.data.url &&
										'' !== e.object.data.url &&
										window.top.open(e.object.data.url),
									(w = !1);
							});
						console.time();
						for (let e = 0; e < 250; e++) d.step(0.02), b();
						console.timeEnd();
						let S = performance.now(),
							M = function (e) {
								let t = e - S;
								(S = e),
									t > 0 && (d.step(Math.min(t, 100) / 1e3), b()),
									g.render(p, v),
									h.update(),
			...
			function (e) {
				e.exports = { nodes: data.nodes, links: data.links };
			},
		]);
		//#endregion
