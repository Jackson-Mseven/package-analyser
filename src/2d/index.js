...
const handleDataStrategy = {
	...
	getRenderData: function (json) {
		// 1、处理深度
		if (json.depth !== 'Infinity') {
			// 局部递归
			dependHash = this.getDepthData(
				JSON.parse(localStorage.getItem('dependHashObj')),
				json.depth
			);
			devPendHash = this.getDepthData(
				JSON.parse(localStorage.getItem('devPendHashObj')),
				json.depth
			);
			dependencyHoop = this.getDepthData(
				JSON.parse(localStorage.getItem('dependencyHoopObj'))[1],
				json.depth
			);
			devDependencyHoop = this.getDepthData(
				JSON.parse(localStorage.getItem('devDependencyHoopObj'))[1],
				json.depth
			);
		} else {
			// 全局递归
		...
		sessionStorage.setItem(
			'dependToVersions',
			JSON.stringify(dependToVersions)
		);
		sessionStorage.setItem(
			'devDependToVersions',
			JSON.stringify(devDependToVersions)
		);
	...
	changed: function (json) {
		...
		localStorage.setItem(
			'dependToVersionsObj',
			JSON.stringify(json.dependToVersions)
		);
		localStorage.setItem(
			'devDependToVersionsObj',
			JSON.stringify(json.devDependToVersions)
		);
		localStorage.setItem(
			'dependencyHoopObj',
			JSON.stringify(json.dependencyHoop)
		);
		localStorage.setItem(
			'devDependencyHoopObj',
			JSON.stringify(json.devDependencyHoop)
		);
