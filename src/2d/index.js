...
const show = (res) => {
  if (typeof res === 'string') { // package.json 没有改变，接收 depth
    console.log('没有改变');
    ...
    if (!isPolyline) {
      ...
      if (showRingBtn.innerHTML == '取消展示') {
        if (inputData == dependHash)
          init(
            isPolyline,
            JSON.parse(localStorage.getItem('dependencyHoop'))
          );
        else if (inputData == devPendHash)
          init(
            isPolyline,
            JSON.parse(localStorage.getItem('devDependencyHoop'))
          );
  ...
  showRingBtn.addEventListener('click', () => {
    ...
    if (isOn) {
      ...
      svg.remove();
      if (inputData == dependHash) {
        ringData = JSON.parse(localStorage.getItem('dependencyHoop'));
        showDepend(json.dependencyHoop[1]);
      } else if (inputData == devPendHash) {
        ringData = JSON.parse(localStorage.getItem('devDependencyHoop'));
      ...
      if (ulElement) {
        ...
        li.addEventListener('dblclick', (e) => {
          ...
          if (RefFlag == 1) {
            ...
            JSON.parse(localStorage.getItem('dependencyHoop')).nodes.forEach(
              (node) => {
                cachedX = node.x;
                cachedY = node.y;
                if (node.id == item) updatePosition(node.index);
            ...
            JSON.parse(
              localStorage.getItem('devDependencyHoop')
            ).nodes.forEach((node) => {
              cachedX = node.x;
              cachedY = node.y;
              if (node.id == item) updatePosition(node.index);
